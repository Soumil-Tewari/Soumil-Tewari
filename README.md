<div align="center">

<img src="https://capsule-render.vercel.app/api?type=blur&height=250&section=header&text=Soumil%20Tewari&fontSize=52&fontColor=FFFFFF&fontAlignY=38&desc=BUILDING%20SOFTWARE%20THAT%20ACTUALLY%20SHIPS&descAlignY=61&descSize=16&animation=fadeIn&color=0:0B0B14,45:35136F,75:7D2AE8,100:00C4CC" width="100%"/>

<br>

<img src="https://readme-typing-svg.demolab.com/?font=JetBrains+Mono&size=18&duration=3000&pause=1000&color=00C4CC&center=true&vCenter=true&width=760&lines=Computer+Science+%40+VIT+Vellore;Mobile+%7C+Web+%7C+Desktop+%7C+Cloud;From+idea+to+production;Build+%E2%86%92+Ship+%E2%86%92+Learn+%E2%86%92+Repeat" alt="Typing SVG">

<br><br>

<a href="https://soumiltewari.netlify.app"><img src="https://img.shields.io/badge/PORTFOLIO-7D2AE8?style=for-the-badge&labelColor=0B0B14"></a>
<a href="https://github.com/Soumil-Tewari"><img src="https://img.shields.io/badge/GITHUB-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=github&logoColor=white"></a>
<a href="https://www.linkedin.com/in/soumiltewari/"><img src="https://img.shields.io/badge/LINKEDIN-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=linkedin&logoColor=white"></a>
<a href="https://twitter.com/SoumilTewari"><img src="https://img.shields.io/badge/X-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=x&logoColor=white"></a>

</div>

<br>

<table align="center">
<tr>
<td align="center" width="33%">

5+

Apps shipped

</td>
<td align="center" width="33%">

5K+

Math Magic downloads

</td>
<td align="center" width="33%">

9.19

CGPA / 10

</td>
</tr>
</table>

<hr>

👋 Hey, I'm Soumil

I'm a Computer Science student at VIT Vellore and a developer who enjoys taking products all the way from an idea to something people can actually install, use and rely on.

My favourite kind of project is not a tutorial clone.

It's the one where I have to figure out:

frontend → backend → database → infrastructure → deployment → users

I've been building software since 2021, starting with Android applications and gradually moving into full-stack systems, developer tools, Windows applications, cloud infrastructure and AI-powered products.

I don't just like writing code. I like shipping software.

<hr>

⚡ How I Build

<div align="center">

IDEA → DESIGN → BUILD → DATABASE + API → CLOUD → DEPLOY → USERS → DEBUG → SHIP AGAIN

</div>

I enjoy working across the stack because the interesting problems usually live between the layers.

<hr>

🚀 Selected Work

📓 Cahier

A full-stack LaTeX IDE built for Windows

Cahier combines a modern React/TypeScript editor with a cloud compilation pipeline and a native Windows shell.

Technology

React TypeScript Monaco Editor Node.js BullMQ Redis Docker XeLaTeX Socket.IO WinUI 3 C# WebView2 Oracle Cloud

What I built

Multi-file LaTeX workspace

Monaco-based code editor

File tree and project management

IndexedDB persistence

PDF and media previews

Integrated compilation terminal

ZIP project export

Dockerized XeLaTeX compilation

BullMQ + Redis job processing

Real-time compiler logs

Native Windows WebView2 integration

Native title-bar integration

Production cloud deployment

MSIX packaging

Microsoft Store release

<a href="https://apps.microsoft.com/detail/9PGNBF9CVQ7V?hl=en-us&gl=US&ocid=pdpshare"><img src="https://img.shields.io/badge/OPEN%20CAHIER%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=microsoft&logoColor=white"></a>

<hr>

📈 Quantaroo

Finance + AI for everyday market research

A cross-platform mobile application combining stock analytics, interactive charts and AI-summarized financial news.

Technology

React Native FastAPI PostgreSQL Firebase Gemini SDK Docker GitHub Actions Oracle Cloud AdMob

What I built

Daily stock analytics

Interactive trading charts

Financial news aggregation

AI-generated news summaries

Scheduled data processing

PostgreSQL-backed financial data

Firebase authentication

Push notifications

Containerized backend

Cloud deployment

Automated CI/CD

AdMob monetization

The entire pipeline:

Market Data → Processing → FastAPI → PostgreSQL → React Native → User

<a href="https://play.google.com/store/apps/details?id=com.techxappsbusiness.Quantaroo"><img src="https://img.shields.io/badge/GOOGLE%20PLAY%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=googleplay&logoColor=white"></a>
<a href="https://apps.samsung.com/appquery/appDetail.as?appId=com.techxappsbusiness.Quantaroo"><img src="https://img.shields.io/badge/GALAXY%20STORE%20↗-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=samsung&logoColor=white"></a>

<hr>

🧮 Math Magic

The project that started it all

Started in 2021, Math Magic is an Android educational application covering topics including exponents, trigonometry and measurements.

Technology

Java, XML, Android SDK, GraphView, AdMob

5,000+ downloads

Available across:

Google Play · Samsung Galaxy Store · Amazon Appstore

What started as a student project became a real product, reached users across multiple stores and kept evolving over the years.

<a href="https://play.google.com/store/apps/details?id=com.techxappsbusiness.mathmagic"><img src="https://img.shields.io/badge/GOOGLE%20PLAY%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=googleplay&logoColor=white"></a>
<a href="https://apps.samsung.com/appquery/appDetail.as?appId=com.techxappsbusiness.mathmagic"><img src="https://img.shields.io/badge/GALAXY%20STORE%20↗-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=samsung&logoColor=white"></a>
<a href="https://www.amazon.in/Math-Magic-Exponents-measurements-trigonometry/dp/B09R1X7GN9"><img src="https://img.shields.io/badge/AMAZON%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=amazon&logoColor=white"></a>

<hr>

📝 Slate

Thoughts & Notes

Capture Thoughts Quickly • Clean & Distraction-Free

A clean and lightweight notes app built with React Native and Expo.

Create, manage, share and store notes locally using AsyncStorage. Designed around Material Design principles for a focused mobile experience.

Technology

React Native, Expo, AsyncStorage, JavaScript, React Native Paper

<a href="https://play.google.com/store/apps/details?id=com.techxappsbusiness.slate"><img src="https://img.shields.io/badge/VIEW%20ON%20GOOGLE%20PLAY%20↗-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=googleplay&logoColor=white"></a>

<hr>

⏱️ Productivity Time Tracker

A native Windows productivity application

Built around one simple idea:

Make consistency visible.

Technology

C#, XAML, WinUI 3, Windows App SDK, SQLite, WebView2, LiveCharts

Features

Work sessions

Daily targets

Streaks

Productivity goals

Analytics

Weekly, monthly and yearly insights

Local-first storage

In-app music

Fully offline, with no cloud dependency.

<a href="https://apps.microsoft.com/detail/9p7746r09rd6"><img src="https://img.shields.io/badge/OPEN%20PRODUCTIVITY%20TRACKER%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=microsoft&logoColor=white"></a>

<hr>

🧰 Tech Stack

<div align="center">

Languages

<img src="https://skillicons.dev/icons?i=python,cpp,js,ts,java,cs,c,sql&theme=dark&perline=8">

<br><br>

Frameworks & Platforms

<img src="https://skillicons.dev/icons?i=react,nodejs,express,fastapi,android&theme=dark&perline=6">

<br><br>

Infrastructure

<img src="https://skillicons.dev/icons?i=postgresql,redis,docker,nginx,firebase,githubactions,oracle&theme=dark&perline=7">

</div>

<br>

Also working with

Monaco Editor · BullMQ · Socket.IO · XeLaTeX · WebView2 · Dexie · GraphView · LiveCharts · Gemini SDK

<hr>

🏗️ Systems I Like Building

                         ┌─────────────────────┐
                         │     USER / APP      │
                         └──────────┬──────────┘
                                    │
                                    ▼
                         ┌─────────────────────┐
                         │      REST API       │
                         └──────────┬──────────┘
                                    │
                         ┌──────────┴──────────┐
                         │                     │
                         ▼                     ▼
                ┌────────────────┐    ┌────────────────┐
                │    Backend     │    │ Background Jobs│
                │ Node / FastAPI │    │ BullMQ / Redis │
                └───────┬────────┘    └───────┬────────┘
                        │                     │
                        ▼                     ▼
                ┌────────────────┐    ┌────────────────┐
                │   PostgreSQL   │    │   Processing   │
                └───────┬────────┘    └────────────────┘
                        │
                        ▼
                ┌────────────────┐
                │ Docker / Cloud │
                └────────────────┘

Infrastructure I've worked with

Docker Nginx Linux HTTPS DNS Redis BullMQ GitHub Actions Oracle Cloud

localhost is not production.

<hr>

📚 Beyond Software

Python Programming Handbook for Beginners

A beginner-focused Python programming book that I wrote and self-published through Amazon KDP.

I published it when I was 16.

<a href="https://www.amazon.in/dp/B0BKL9KY4J"><img src="https://img.shields.io/badge/READ%20THE%20BOOK%20↗-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=amazon&logoColor=white"></a>

<hr>

🎓 Currently

B.Tech Computer Science & Engineering
VIT Vellore · 2025 → 2029

CGPA: 9.19 / 10

Interested in

Software Engineering · Systems · AI / LLMs · Data · Cloud Infrastructure · Developer Tools · Research

<hr>

📊 GitHub Activity

<div align="center">

<img src="./profile/streak.svg" width="520" alt="GitHub contribution streak">

</div>

<hr>

🏆 By the Numbers

<div align="center">

<table>
<tr>
<td align="center" width="20%"><strong>2021</strong><br><sub>Started building</sub></td>
<td align="center" width="20%"><strong>5+</strong><br><sub>Apps shipped</sub></td>
<td align="center" width="20%"><strong>5K+</strong><br><sub>Downloads</sub></td>
<td align="center" width="20%"><strong>9.19</strong><br><sub>CGPA / 10</sub></td>
<td align="center" width="20%"><strong>1</strong><br><sub>Technical book</sub></td>
</tr>
</table>

</div>

<hr>

🌐 Let's Connect

<div align="center">

<a href="https://soumiltewari.netlify.app"><img src="https://img.shields.io/badge/PORTFOLIO-7D2AE8?style=for-the-badge&labelColor=0B0B14"></a>
<a href="https://www.linkedin.com/in/soumiltewari/"><img src="https://img.shields.io/badge/LINKEDIN-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=linkedin&logoColor=white"></a>
<a href="https://twitter.com/SoumilTewari"><img src="https://img.shields.io/badge/X-7D2AE8?style=for-the-badge&labelColor=0B0B14&logo=x&logoColor=white"></a>
<a href="https://www.instagram.com/soumiltewari/"><img src="https://img.shields.io/badge/INSTAGRAM-00C4CC?style=for-the-badge&labelColor=0B0B14&logo=instagram&logoColor=white"></a>

<br><br>

Build. Ship. Learn. Repeat.

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&height=150&section=footer&color=0:00C4CC,45:7D2AE8,75:35136F,100:0B0B14" width="100%">

</div>
